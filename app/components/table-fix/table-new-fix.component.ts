...
@Component({
    selector: 'table-new-fix',
    templateUrl: './table-new-fix.component.html',
    styleUrls: ['./table-new-fix.component.css']
})

export class TableNewFixedComponent implements AfterViewInit{
    ...
    ngAfterViewInit(){
        ...
        console.log(this.thead, this.tbody, container.nativeElement);
        this.relayout();
        container.nativeElement.addEventListener('scroll', this.headerScroll.bind(this));
    }

    headerScroll(e){
        // Fix thead and first column on scroll
        //this.thead.style.transform = 'translate3d(0,' + e.target.scrollTop + 'px,0)';
        [].slice.call(this.thead.querySelectorAll('tr > .fixTH'))
        .forEach(function(td, i) {
            td.style.transform =  'translate3d(0,' + e.target.scrollTop + 'px,0)';
        });
        ...
        [].slice.call(this.tbody.querySelectorAll('tr > .fixTD'))
        .forEach(function(td, i) {
            td.style.transform = hTransform;
        });
      };
